...
class Environment(object):
	...
	def _get_state(self):
		...
		# return np.concatenate([[D,angle],self.velocities,[self.closest_point,self.closest_direction]])

		lidar = np.asarray([self.closest_point,self.closest_direction])
		target = np.asarray([D,angle])
	...
	def callback_pose(self,data):
		...
		# self.robot_pose = np.asarray([data.pose.pose.position.x,data.pose.pose.position.y,data.pose.pose.position.z,angles[2]])
		self.robot_pose = np.asarray([data.pose.pose.position.x,data.pose.pose.position.y])
	...
	def callback_laser(self, data):
		...
		# self.closest_direction = np.deg2rad(np.argmin(self.laser))
