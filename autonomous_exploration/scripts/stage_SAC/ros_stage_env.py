...
class StageEnvironment:
	def __init__(self,args,flag_rnd):
		...
		self.freeMap_size = 0
		self.frontier_polar = np.asarray([])
		self.relative_frontierPos = np.asarray([])
	...
	def reset_pose(self, data):
		...
		new_state = np.asarray([self.frontier_polar, self.map],dtype=object)
	...
	def relative_angle(self, r, f):
		angle_r_to_f = math.atan2((f[1] - r[1]), (f[0] - r[0]))

		diff = (r[2] - angle_r_to_f) % (math.pi * 2)
		if diff >= math.pi:
			diff -= math.pi * 2
	...
	def compute_polarCoord(self, frontier, robot_pose):
		angle = self.relative_angle(robot_pose,frontier)
		distancy = _dist([robot_pose[0],robot_pose[1]],[frontier[0],frontier[1]])
		
		return np.array([distancy,angle])
	...
	def callback_frontier(self, data):
		...
		for i in range(len(data.clusters)):
			...
			self.relative_frontierPos.append(self.compute_polarCoord([data.clusters[i].x,data.clusters[i].y],self.robot_pose))

		# k = 0
		for idx in np.argsort(dist):
			self.frontier.append(self.frontier_aux[idx])
			self.frontier_polar.append(self.relative_frontierPos[idx])
			# k +=1

		self.frontier = np.asarray(self.frontier)
		self.frontier_polar = np.asarray(self.frontier_polar,dtype=np.float32)
	...
	def callback_image(self, data):
		...
		self.map = np.asarray([img],dtype=np.float32)

		self.observation_space = np.asarray([self.frontier_polar,self.map],dtype=object)
