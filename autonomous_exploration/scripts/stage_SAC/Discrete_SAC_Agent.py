...
class SACAgent:
    ...
    ALPHA_INITIAL = 1.
    REPLAY_BUFFER_BATCH_SIZE = 256 #100
    DISCOUNT_RATE = 0.99
    LEARNING_RATE = 10 ** -4
    SOFT_UPDATE_INTERPOLATION_FACTOR = 0.01

    def __init__(self, state_dim,action_space, chkpt_dir='./models/'):

        rospack = rospkg.RosPack()
        pkg_path = rospack.get_path('autonomous_exploration')
        file_path = pkg_path + "/scripts/stage_SAC/models/"
        self.checkpoint_dir = file_path
    ...
    def save_net(self):
        print("Models Saved")
        torch.save(self.critic_local.state_dict(), os.path.join(self.checkpoint_dir, 'critic_local_sac'))
        torch.save(self.critic_local2.state_dict(), os.path.join(self.checkpoint_dir, 'critic_local2_sac'))
        torch.save(self.critic_target.state_dict(), os.path.join(self.checkpoint_dir, 'critic_target_sac'))
        torch.save(self.critic_target2.state_dict(), os.path.join(self.checkpoint_dir, 'critic_target2_sac'))
        torch.save(self.actor_local.state_dict(), os.path.join(self.checkpoint_dir, 'actor_local_sac'))

    def load_net(self):
        self.critic_local.load_state_dict(torch.load(os.path.join(self.checkpoint_dir, 'critic_local_sac')))
        self.critic_local2.load_state_dict(torch.load(os.path.join(self.checkpoint_dir, 'critic_local2_sac')))
        self.critic_target.load_state_dict(torch.load(os.path.join(self.checkpoint_dir, 'critic_target_sac')))
        self.critic_target2.load_state_dict(torch.load(os.path.join(self.checkpoint_dir, 'critic_target2_sac')))
        self.actor_local.load_state_dict(torch.load(os.path.join(self.checkpoint_dir, 'actor_local_sac')))
    ...
    def get_action_probabilities(self, state):
        state_tensor = []
        for k in state:
            state_tensor.append(torch.from_numpy(k).float().unsqueeze(0))
