...
class Map:
    def __init__(self, size, resolution):
        ...
        self.size = (np.array(size)/self.resolution).astype(int)
    ...
    def inverse_scanner(self, pose, meas_r):
        alpha = 1.5 # Width of an obstacle (distance about measurement to fill in).
        ...
        for i in range(w):
            for j in range(h):
                if(self.data[i,j] == 0.5 ):
                    ...
                    image[i,j] = 205
                elif(self.data[i,j] >= 0.7):
                    self.map_increse += 1
                    # image[i,j] = [0,0,0]
                    image[i,j] = 0
                elif(self.data[i,j] <= 0.3):
                    self.map_increse += 1
