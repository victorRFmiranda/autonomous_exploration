...
class FitnessData:
    ...
    def calculate_reward(self, robot_x: float, robot_y: float, robot_orientation: float, env_done: bool, linear_velocity: float, angular_velocity: float, lidar_data):
        """
        ...
        # avoid obstacle
        min_lidar = min(lidar_data)
        if min_lidar < 0.02:
            reward -= 3

        #reward ~ [-18, 12 ]
        if env_done:
            reward = -60 #- distance_robot_to_end / distance_start_to_end * 100
            done = True
        elif distance_robot_to_end < self._node_data.get_node_end().radius():
            reward = 60
            done = self._handle_terminate_at_end()
        else: # fast to the target
            reward-=1
